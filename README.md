...
| 뉴스 | `_data/news.yml` |
| 언론 보도 · 산업/연구 동향 기사 | `_data/press.yml` |
...
**구성원 · 주요 논문 · 모집 공고**는 위 `_data/` 파일에서 자동으로 불러오므로 홈페이지와 한 번에 갱신됩니다.
수상 실적 · 연구 과제 · 언론 보도는 홈페이지(소식 · 연구 페이지)에만 두고, 가이드의 '연구실 소개' 맨 아래에서 링크합니다.
생활 규칙, 연구장비, 연구 방법 같은 가이드 전용 내용은 이 파일에서 직접 고칩니다.
